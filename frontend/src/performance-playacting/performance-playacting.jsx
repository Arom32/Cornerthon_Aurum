...
import './performance-playacting.css'; // CSS 파일명 수정
...
const Playacting = () => {
        ...
        const fetchData = async () => {
            try {
                ...
                const performanceData = data
                    .filter(post => post.category === '연극') // 카테고리 필터 수정
                    .map(post => ({
                        id: post.id,
                        imgUrl: post.poster,
                        title: post.prfnm
    ...
    const renderPlayactingCards = () => {
        ...
        for (let i = 0; i < totalCards; i++) {
            if (performance[i]) {
                cards.push(
                    <article 
                        key={`playacting-${performance[i].id}`} 
    ...
    return (
        <div className="container">
            ...
            {/* 공연 설명 */}
            <nav className="ex-playacting">오늘의 연극 정보를 안내해 드립니다.</nav>
            
            {/* 연극 목록 섹션 */}
            <nav className="all-playacting">
                <div className="all-grid">
                    {renderPlayactingCards()}
                </div>
            </nav>
        </div>
    );
